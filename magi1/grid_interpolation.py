...
class GridInterpolationKernel(GridKernel):
    ...
    * :math:`\mathbf{w_{x_1}}` and :math:`\mathbf{w_{x_2}}` are sparse vectors based on
    ...
    def __init__(
        self,
        base_kernel: Kernel,
        grid_size: Optional[Union[int, List[int]]] = None,
        num_dims: Optional[int] = None,
        grid_bounds: Optional[Tuple[float, float]] = None,
        active_dims: Optional[Tuple[int, ...]] = None,
        grid: Optional[Union[torch.Tensor,List[torch.Tensor]]] = None,
        interpolation_orders: Optional[int] = 3,
    ):
        if (grid is not None):
            if (not isinstance(grid, list)):
                grid = [grid]
            grid = [grid[i].squeeze() for i in range(len(grid))]
            ...
            if num_dims is None:
                ...
                raise RuntimeError(
                    ...
                    "grid ({})".format(num_dims, len(grid))
                    )

            has_initialized_grid = 1
            grid_is_dynamic = False
            grid_sizes = [grid[i].size(0) for i in range(len(grid))]
            grid_bounds = tuple((grid[i].min(),grid[i].max()) for i in range(len(grid)))

        else:
            if (grid_size is None):
                raise RuntimeError("Either grid or grid_size must be provided.")
            ...
            grid = create_grid(grid_sizes, grid_bounds)

        if (interpolation_orders != 3 or interpolation_orders != 5):
            RuntimeError("Only support interpolation orders 3 and 5.")
        ...
        self.interpolation_points = range(-2,2) if (self.interpolation_orders == 3) else range(-3,3)
    ...
    def _tight_grid_bounds(self):
        padding = (self.interpolation_orders + 1) / 2 + 0.01
        grid_spacings = tuple((bound[1] - bound[0]) / self.grid_sizes[i] for i, bound in enumerate(self.grid_bounds))
        return tuple(
            (bound[0] + padding * spacing, bound[1] - padding * spacing)
            for bound, spacing in zip(self.grid_bounds, grid_spacings)
        )

    def _compute_grid(self, inputs, last_dim_is_batch=False, derivative=False):
        ...
        interp_indices, interp_values = Interpolation().interpolate(self.grid, inputs, interp_points=self.interpolation_points, interp_orders=self.interpolation_orders, derivative=derivative)
    ...
    def forward(self, x1, x2, diag=False, last_dim_is_batch=False, **params):
        ...
        if self.grid_is_dynamic:  # This is true if a grid_bounds wasn't passed in
            ...
            if update_grid:
                padding = (self.interpolation_orders + 1) / 2 + 0.01
                grid_spacings = tuple(
                    (x_max - x_min) / (gs - 2 * padding) for gs, x_min, x_max in zip(self.grid_sizes, x_mins, x_maxs)
                )
                self.grid_bounds = tuple(
                    (x_min - padding * spacing, x_max + padding * spacing)
    ...
    def dC(self, x1, x2, last_dim_is_batch=False, dx1=False, dx2=False, **params):
        x1 = x1.squeeze()
        x2 = x2.squeeze()
        ...
        left_interp_indices, left_interp_values = self._compute_grid(x1, last_dim_is_batch, derivative=dx1)
        right_interp_indices, right_interp_values = self._compute_grid(x2, last_dim_is_batch, derivative=dx2)
    ...
    @torch.no_grad()
    def C(self, x1, x2, last_dim_is_batch=False, **params):
        return (self.dC(x1, x2, last_dim_is_batch=last_dim_is_batch, dx1=False, dx2=False, **params))

    @torch.no_grad()
    def dCdx1(self, x1, x2, last_dim_is_batch=False, **params):
        return (self.dC(x1, x2, last_dim_is_batch=last_dim_is_batch, dx1=True, dx2=False, **params))

    @torch.no_grad()
    def dCdx2(self, x1, x2, last_dim_is_batch=False, **params):
        return (self.dC(x1, x2, last_dim_is_batch=last_dim_is_batch, dx1=False, dx2=True, **params))

    @torch.no_grad()
    def d2Cdx1dx2(self, x1, x2, last_dim_is_batch=False, **params):
        return (self.dC(x1, x2, last_dim_is_batch=last_dim_is_batch, dx1=True, dx2=True, **params))